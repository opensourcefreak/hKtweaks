 ...
 */
public class BusMifFragment extends RecyclerViewFragment {
    ...
    protected void addItems(List<RecyclerViewItem> items) {
        ...
        final List<String> freqs = VoltageMif.getFreqs();
        final List<String> voltages = VoltageMif.getVoltages();
        final List<String> voltagesStock = VoltageMif.getStockVoltages();
        ...
        if (freqs != null && voltages != null && voltagesStock != null && freqs.size() == voltages.size()) {
            ...
            for (float i = mVoltMinValue; i < (mVoltMaxValue + mVoltStep); i += mVoltStep) {
                String global = String.valueOf(i / VoltageMif.getOffset());
                progress.add(global);
            }
            ...
            Boolean enableGlobal = AppSettings.getBoolean("busMif_global_volts", true, getActivity());
            ...
            voltControl.addOnSwitchListener((switchView, isChecked) -> {
                if (isChecked) {
                    AppSettings.saveBoolean("busMif_global_volts", true, getActivity());
                    AppSettings.saveBoolean("busMif_individual_volts", false, getActivity());
                    reload();
                } else {
                    AppSettings.saveBoolean("busMif_global_volts", false, getActivity());
                    AppSettings.saveBoolean("busMif_individual_volts", true, getActivity());
                    AppSettings.saveInt("busMif_seekbarPref_value", mDefZeroPosition, getActivity());
                    reload();
                }
            });
    ...
    private void seekbarProfInit(SeekBarView seekbar, final List<String> freqs, final List<String> voltages,
        ...
        Boolean enableSeekbar = AppSettings.getBoolean("busMif_global_volts", true, getActivity());
        int global = AppSettings.getInt("busMif_seekbarPref_value", mDefZeroPosition, getActivity());
        ...
        seekbar.setOnSeekBarListener(new SeekBarView.OnSeekBarListener() {
            ...
            public void onStop(SeekBarView seekBarView, int position, String value) {
                for (int i = 0; i < voltages.size(); i++) {
                    ...
                    VoltageMif.setVoltage(freq, volt, getActivity());
                    AppSettings.saveInt("busMif_seekbarPref_value", position, getActivity());
                }
                getHandler().postDelayed(BusMifFragment.this::reload, 200);
            }
            @Override
    ...
    private void seekbarInit(SeekBarView seekbar, final String freq, String voltage,
        ...
        int mOffset = VoltageMif.getOffset();
        ...
        Boolean enableSeekbar = AppSettings.getBoolean("busMif_individual_volts", false, getActivity());
        ...
        seekbar.setOnSeekBarListener(new SeekBarView.OnSeekBarListener() {
            ...
            @Override
            public void onStop(SeekBarView seekBarView, int position, String value) {
                VoltageMif.setVoltage(freq, value, getActivity());
    ...
    private void reload() {
        List<String> freqs = VoltageMif.getFreqs();
        List<String> voltages = VoltageMif.getVoltages();
        List<String> voltagesStock = VoltageMif.getStockVoltages();
